...
class MultiplayerJumbleGame:
    def __init__(self, root):
        self.root = root
        self.root.title("Multiplayer Word Jumble Game")
        ...
        tk.Label(root, text="Word Jumble Game", font=('Arial', 26, 'bold'), bg="#F0F8FF").pack(pady=10)
        tk.Label(root, textvariable=self.jumbled_word, font=('Courier', 32, 'bold'), fg='blue', bg="#F0F8FF").pack(pady=10)
        tk.Label(root, textvariable=self.input_label, font=('Arial', 18), bg="#F0F8FF").pack(pady=5)

        self.entry_frame = tk.Frame(root, bg="#F0F8FF")
        ...
        tk.Label(root, textvariable=self.status_text, font=('Arial', 16), bg="#F0F8FF").pack(pady=5)
        tk.Label(root, textvariable=self.timer_text, font=('Arial', 16), fg='red', bg="#F0F8FF").pack(pady=2)
        tk.Label(root, textvariable=self.score_text, font=('Arial', 16), bg="#F0F8FF").pack(pady=5)

        self.score_text.set("Score - Player 1: 0 | Player 2: 0")
        self.reset_round()
    ...
    def reset_round(self):
        ...
        self.timer_text.set("")
        self.input_label.set(f"Player {self.player_turn}, enter a word:")
    ...
    def handle_submit(self):
        ...
        if not self.original_word:
            if not input_text.isalpha():
                self.status_text.set("Please enter a valid word.")
                return
    ...
    def check_guess(self, guess):
        ...
        if guess == self.original_word:
            ...
            self.status_text.set(f"Wrong! The word was: {self.original_word}. Player {3 - self.player_turn} loses a point.")
            self.score[3 - self.player_turn] = max(0, self.score[3 - self.player_turn] - 1)
